...
def qualitative_validation():
    VAL_PATH = './IL/classified_images/' #"validation_predictions"
    ...
    trainRCNN = pd.read_csv('IL/IL_val_labels.csv', sep=",")
    ...
    for imageFileName in image_names:    
        ...
        for _,row in trainRCNN[trainRCNN.filename == imageFileName].iterrows():
            ...
            ax.add_patch(rect)   
            if not os.path.exists("IL/validation_qualitative"):
                os.makedirs("IL/validation_qualitative")

            fig.savefig('IL/validation_qualitative/' + os.path.splitext(imageFileName)[0] + ".jpg", dpi=90, bbox_inches='tight')
